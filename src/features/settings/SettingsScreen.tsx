...
export function SettingsScreen({ onSignOut, onExplorePress }: { onSignOut: () => void; onExplorePress: () => void }) {
  ...
  const saveProfileHandler = useCallback(async () => {
    ...
    try {
      dispatch(setProfile({ name, email, avatarUri: editAvatarUri }));
      setActiveSheet(null);
    } catch (error) {
  ...
  }, [editProfileName, editProfileEmail, editAvatarUri, dispatch]);
  ...
  return (
    <View style={styles.tabContent}>
      ...
      <Modal animationType="slide" transparent visible={activeSheet !== null} onRequestClose={() => setActiveSheet(null)}>
              ...
              <>
                ...
                <TouchableOpacity style={styles.profileIdentity} activeOpacity={0.8} onPress={pickProfileImage}>
                  <View style={styles.profileAvatar}>
                    {editAvatarUri ? (
                      <Image source={{ uri: editAvatarUri }} style={styles.profileAvatarPhoto} />
                    ) : (
                      <Text style={styles.profileAvatarText}>{getInitials(editProfileName)}</Text>
                    )}
                  </View>
                  <View>
                    <Text style={styles.profileName}>{editProfileName || 'Add your name'}</Text>
                    <Text style={styles.profileEmail}>Tap to change photo</Text>
                  </View>
                </TouchableOpacity>
                <Text style={styles.inputLabel}>DISPLAY NAME</Text>
                <TextInput value={editProfileName} onChangeText={v => { setEditProfileName(v); setProfileNameError(''); }} style={styles.textInput} placeholder="Your name" />
                {!!profileNameError && <Text style={styles.inputError}>{profileNameError}</Text>}
                <Text style={styles.inputLabel}>EMAIL</Text>
                <TextInput value={editProfileEmail} onChangeText={v => { setEditProfileEmail(v); setProfileEmailError(''); }} style={styles.textInput} keyboardType="email-address" autoCapitalize="none" />
                {!!profileEmailError && <Text style={styles.inputError}>{profileEmailError}</Text>}
                <TouchableOpacity style={[styles.primarySheetButton, isSavingProfile && styles.primarySheetButtonDisabled]} disabled={isSavingProfile} onPress={saveProfileHandler}>
...
const styles = StyleSheet.create({
  ...
  dangerSheetButton: { marginTop: 12, height: 50, borderRadius: 14, backgroundColor: '#FEE2E2', borderWidth: 1, borderColor: '#FECACA', alignItems: 'center', justifyContent: 'center' },
